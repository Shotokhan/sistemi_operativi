...
void produci_alta_prio(PriorityProdCons *p){
	enter_monitor(&(p->m));
	printf("Prod ad alta priorita' %d schedulato nel monitor\n", getpid());
	// condizione buffer pieno
	if(p->numOccupati == DIM){
...
void produci_bassa_prio(PriorityProdCons *p){
	enter_monitor(&(p->m));
	printf("Prod a bassa priorita' %d schedulato nel monitor\n", getpid());
	// condizione buffer pieno
	if(p->numOccupati == DIM || (p->m).cond[H_PROD] > 0){
		printf("Prod a bassa priorita' %d, in attesa del buffer\n", getpid());
		while(p->numOccupati == DIM || (p->m).cond[H_PROD] > 0){
